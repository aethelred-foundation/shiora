...
export class PgDocumentStore implements DocumentStorePort {
  ...
  async put(doc: StoredDocument): Promise<void> {
    await this.client.query(
      `INSERT INTO documents (collection, owner_key, id, sealed, deleted, version, deleted_at, updated_at)
      ...
      [doc.collection, doc.ownerKey, doc.id, JSON.stringify(doc.sealed), doc.deleted, doc.version ?? 1, doc.deletedAt ?? null],
    );
  }
  ...
  async findById(
    ...
    const { rows } = await this.client.query<DocumentRow>(
      `SELECT collection, owner_key, id, sealed, deleted, version, deleted_at FROM documents
