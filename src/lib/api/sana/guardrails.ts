...
// code — they are the product, not the LLM behind them.
//
// Three layers:
//   1. Input screening — intercept medical emergencies and self-harm crises and
//      return a fixed safety response WITHOUT calling the LLM. A language model
//      must never be the thing standing between a user and emergency care.
//   2. System prompt — a hard, explicit non-diagnostic contract for the model.
...
const CRISIS = /suicid|kill myself|killing myself|end my life|want to die|hurt myself|harm myself|self[- ]?harm/i;

// Acute medical emergency.
const EMERGENCY = /chest pain|can'?t breathe|cannot breathe|trouble breathing|severe bleeding|having a stroke|overdose|anaphyla|unconscious|not breathing/i;

const CRISIS_RESPONSE = 'It sounds like you may be going through a crisis, and you are not alone — '
...
const EMERGENCY_RESPONSE = 'This may be a medical emergency. Please call your local emergency number '
...
/** Intercept crises/emergencies before the LLM. Otherwise allow the message through. */
...
/** The hard non-diagnostic contract handed to the model on every turn. */
...
const DISCLAIMER = 'SANA provides general information only and is not a substitute for professional '
