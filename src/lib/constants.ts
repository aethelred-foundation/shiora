...
/** Neutral metadata for configured inference workloads. */
export const INFERENCE_WORKLOADS = [
  {
    ...
    type: 'time-series',
    accuracy: 96.2,
    description: 'Evaluates menstrual cycle timing patterns',
  },
  {
    ...
    accuracy: 93.8,
    description: 'Detects unusual health-pattern deviations',
  },
  {
    id: 'fertility-window',
    name: 'Fertility Window Analysis',
    version: 'v1.7',
    type: 'risk-scoring',
    accuracy: 91.5,
    description: 'Evaluates fertile-window indicators',
  },
  {
    id: 'health-guidance',
    ...
    accuracy: 94.7,
    description: 'Produces non-diagnostic health guidance',
  },
] as const;
...
export const CONSENT_SCOPES = [
  ...
  { id: 'verified_inferences', label: 'Verified Inferences', icon: 'Brain' },
  { id: 'full_access', label: 'Full Access', icon: 'Shield' },
] as const;

/** Health-assistant suggested prompts */
...
export const SECONDARY_NAV_LINKS = [
  { href: '/api/openapi', label: 'API Schema', icon: 'FileText' },
  { href: '/api/system/status', label: 'Service Status', icon: 'Activity' },
  { href: '/api/health/ready', label: 'Readiness', icon: 'ShieldCheck' },
] as const;

/** Tertiary navigation links (More dropdown) */
export const TERTIARY_NAV_LINKS = [
  { href: '/.well-known/security.txt', label: 'Security Contact', icon: 'ShieldCheck' },
] as const;
