...
export function TopNav() {
  ...
  const { formattedBlockHeight, isConnected } = useNetwork();
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-xs">
              <LiveDot color={isConnected ? 'bg-emerald-500' : 'bg-slate-400'} />
              <span className="text-slate-500">Block</span>
              <span className="font-mono font-medium text-slate-700">{formattedBlockHeight}</span>
            </div>
...
export function Footer() {
  const { state, formattedTps, isConnected } = useNetwork();
  ...
  const sections = [
    {
      ...
      links: [
        ...
        { label: 'FHIR Bridge', href: '/fhir' },
      ],
    },
    {
      title: 'Resources',
      links: [
        { label: 'API Schema', href: '/api/openapi' },
        { label: 'Service Status', href: '/api/system/status' },
        ...
        <div className="border-t border-slate-100 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-xs text-slate-400">
            <div className="flex items-center gap-1.5">
              <LiveDot color={isConnected ? 'bg-emerald-500' : 'bg-slate-400'} />
              <span>{isConnected ? 'Network Connected' : 'Network Unavailable'}</span>
            </div>
            <span>|</span>
            <span>Epoch {state.epoch ?? 'Unavailable'}</span>
            <span>|</span>
            <span>
              {formattedTps}
              {state.tps === null ? '' : ' TPS'}
