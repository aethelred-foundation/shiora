...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const checks = controlNames.map((name, i) => ({
      ...
      status:
        i < 10
          ? seededPick(SEED + i * 72, ['pass', 'pass', 'pass', 'partial'] as const)
          : seededPick(SEED + i * 72, statuses),
      evidence: [`Evidence document ${i + 1}`, `TEE attestation log ${i + 1}`],
      remediation:
        seededPick(SEED + i * 73, statuses) === 'fail'
          ? `Apply corrective controls per ${framework.toUpperCase()} guidelines`
          : undefined,
      lastCheckedAt: Date.now() - seededInt(SEED + i * 74, 1, 30) * 86400000,
      teeVerified: seededInt(SEED + i * 75, 0, 10) > 2,
      attestation:
        seededInt(SEED + i * 76, 0, 10) > 3 ? generateAttestation(SEED + i * 77) : undefined,
    }));
