...
> = {
  hipaa: {
    ...
    version: '2024 Rev',
    description:
      'Health Insurance Portability and Accountability Act — privacy and security standards for PHI',
  },
  gdpr: {
    name: 'GDPR',
    version: 'Art. 9',
  ...
  soc2: {
    ...
    version: 'Type II',
    description:
      'Service Organization Control 2 — trust services criteria for security and availability',
  },
  hitrust: {
    name: 'HITRUST',
    version: 'CSF v11',
...
function generateOverview() {
  const frameworks = FRAMEWORK_IDS.map((id, i) => {
    ...
    const passedControls = seededInt(
      SEED + i * 101,
      Math.round(totalControls * 0.7),
      totalControls,
    );
  ...
  return {
    ...
    overallComplianceScore: Math.round(
      frameworks.reduce((s, f) => s + f.overallScore, 0) / frameworks.length,
    ),
...
function generateViolations() {
  ...
  return Array.from({ length: 8 }, (_, i) => ({
    ...
    assignedTo: seededPick(SEED + i * 307, [
      'Dr. Sarah Chen',
      'Security Team',
      'Compliance Officer',
      'DevOps Lead',
    ...
    status: (i >= 6
      ...
      : i >= 4
        ? 'in_progress'
        : seededPick(SEED + i * 309, ['open', 'in_progress'] as const)) as
      | 'open'
      | 'in_progress'
      | 'resolved'
      | 'accepted_risk',
...
export async function GET(request: NextRequest) {
  const blocked = runMiddleware(request, { requireAdmin: true });
