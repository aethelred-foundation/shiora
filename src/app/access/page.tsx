...
function generateGrants(): AccessGrant[] {
  const statuses: AccessGrant['status'][] = [
    'Active',
    'Active',
    'Active',
    'Expired',
  ...
  return Array.from({ length: 8 }, (_, i) => ({
    ...
    lastAccess:
      statuses[i] === 'Active' ? Date.now() - seededInt(SEED + i * 17, 1, 48) * 3600000 : null,
    accessCount:
      statuses[i] === 'Active' ? seededInt(SEED + i * 19, 3, 47) : seededInt(SEED + i * 19, 0, 15),
...
function GrantDetailModal({
  ...
  const viewBorder = grant.canView
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div
              className={`flex items-center gap-2 p-3 rounded-xl border ${grant.canDownload ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}
            >
              <FileText
                className={`w-4 h-4 ${grant.canDownload ? 'text-emerald-600' : 'text-slate-400'}`}
              />
              <span
                className={`text-sm ${grant.canDownload ? 'text-emerald-700' : 'text-slate-400'}`}
              >
                Download
              </span>
            </div>
            <div
              className={`flex items-center gap-2 p-3 rounded-xl border ${grant.canShare ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}
            >
              <ExternalLink
                className={`w-4 h-4 ${grant.canShare ? 'text-emerald-600' : 'text-slate-400'}`}
              />
              <span className={`text-sm ${grant.canShare ? 'text-emerald-700' : 'text-slate-400'}`}>
                Share
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-slate-50 rounded-xl p-3">
            ...
            <p
              className={`text-sm font-medium ${daysLeft > 0 ? 'text-slate-900' : 'text-red-600'}`}
            >
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Manage provider access to your encrypted health data with blockchain-verified
                permissions
              </p>
            </div>
            ...
            <>
              ...
              <div className="space-y-4">
                {filteredGrants.map((grant) => {
                  ...
                  /* istanbul ignore next -- canView is always true in generated data */
                  const viewPermClass = grant.canView
                    ...
                    >
                      <div className="flex items-start gap-4">
                          ...
                          <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {grant.status === 'Expired'
                                ? 'Expired'
                                : `${daysLeft > 0 ? daysLeft : /* istanbul ignore next */ 0}d remaining`}
                            </span>
                            {grant.lastAccess && (
                              <span className="flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <div
                            className={`w-8 h-8 rounded-lg flex items-center justify-center ${viewPermClass}`}
                          >
                            <Eye className="w-3.5 h-3.5" />
                          </div>
                          <div
                            className={`w-8 h-8 rounded-lg flex items-center justify-center ${grant.canDownload ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-300'}`}
                          >
                            <FileText className="w-3.5 h-3.5" />
                          </div>
                          <div
                            className={`w-8 h-8 rounded-lg flex items-center justify-center ${grant.canShare ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-300'}`}
                          >
                      ...
                      <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-8 h-8 rounded-lg ${TYPE_COLORS[entry.type]} flex items-center justify-center`}
                            >
                        ...
                        <td className="px-5 py-3">
                          <p className="text-sm text-slate-500 max-w-[250px] truncate">
                            {entry.details}
                          </p>
                        </td>
                        <td className="px-5 py-3">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-brand-700">
                  All access grants are recorded on the Aethelred blockchain and enforced through
                  smart contracts. Providers can only decrypt your data within TEE enclaves during
                  their authorized access window. Data never leaves the enclave unencrypted, and
                  every access produces a verifiable audit trail.
                </p>
              </div>
            </div>
          </div>
