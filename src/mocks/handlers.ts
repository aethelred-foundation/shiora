...
export const handlers = [
  ...
  http.delete('*/api/access/:id', ({ params }) => {
    if (!mockGrants.find((g) => g.id === params.id)) {
      return err('NOT_FOUND', 'Grant not found', 404);
    }
  ...
  http.get('*/api/insights/anomalies', () =>
    ok({ anomalies: mockInsights.anomalies, total: mockInsights.anomalies.length }),
  ),
  http.get('*/api/insights/inferences', () => ok({ inferences: [], total: 0 })),

  // ── TEE Status ────────────────────────────────────────────────────────────

  http.get('*/api/tee/status', () =>
    ok({
      status: 'operational',
      platform: 'Intel SGX',
  ...
  http.get('*/api/network/status', () =>
    ok({ blockHeight: 2847391, tps: 2150, epoch: 247, networkLoad: 72, aethelPrice: 1.24 }),
  ),
  ...
  http.post('*/api/chat', async ({ request }) => {
    ...
    return ok(
      {
        id: `msg-${Date.now()}`,
        conversationId: body.conversationId || 'conv-0001',
        role: 'assistant',
        content:
  ...
  http.get('*/api/vault/compartments/:id', ({ params }) =>
    ok({
      id: params.id,
  ...
  http.get('*/api/consent/:id', ({ params }) =>
    ok({
      id: params.id,
      patientAddress: `aeth1${'p'.repeat(38)}`,
      providerAddress: `aeth1${'q'.repeat(38)}`,
  ...
  http.get('*/api/marketplace/stats', () =>
    ok({
      totalListings: 847,
      activeListings: 312,
      totalVolume: 45200,
  ...
  http.get('*/api/wearables/:provider', ({ params }) =>
    ok({
      id: `dev-${params.provider}`,
      provider: params.provider,
      deviceName: `${String(params.provider).replace('_', ' ')} Device`,
      status: 'connected',
      lastSync: Date.now() - 3600000,
  ...
  http.post('*/api/fhir/export', async ({ request }) => {
    const body = (await request.json()) as Record<string, unknown>;
    return ok(
      { id: `export-${Date.now()}`, ...body, status: 'processing', startedAt: Date.now() },
      201,
    );
  }),
  ...
  http.post('*/api/zkp/prove', async ({ request }) => {
    ...
    return ok(
      {
        id: `proof-${Date.now()}`,
        claimType: body.claimType || 'age_range',
  ...
  http.get('*/api/community/circles/:id', ({ params }) =>
    ok({
      id: params.id,
  ...
  http.get('*/api/providers/reputation', () =>
    ok(
      Array.from({ length: 5 }, (_, i) => ({
  ...
  http.get('*/api/providers/:address', ({ params }) =>
    ok({
  ...
  http.get('*/api/xai/shap', () =>
    ok({
      ...
      shapValues: [
        ...
        { feature: 'lh_surge', value: 42, baseValue: 35, contribution: 0.2 },
      ...
      featureImportances: [
        ...
        { feature: 'lh_surge', importance: 0.2, direction: 'positive' },
      ...
      confidence: 96.2,
      explanation:
        'The workload estimates normal ovulation timing based primarily on consistent cycle length and clear temperature shift.',
    }),
  ),

  http.get('*/api/xai/model-cards', () =>
    ok([
      {
        modelId: 'cycle-patterns',
        name: 'Cycle Pattern Analysis',
        version: 'v2.1',
        description: 'Estimates menstrual cycle timing from longitudinal health signals.',
        architecture: 'time-series',
        trainingDataSize: 125000,
        validationAccuracy: 96.2,
        fairnessMetrics: { demographicParity: 0.95, equalizedOdds: 0.92, calibration: 0.97 },
        limitations: ['Requires minimum 3 cycles of data', 'Less accurate for irregular cycles'],
        intendedUse: 'Menstrual cycle timing estimation',
      ...
      },
      {
        ...
        version: 'v1.4',
        description: 'Detects unusual health patterns from longitudinal telemetry.',
        architecture: 'anomaly-detection',
        trainingDataSize: 85000,
        validationAccuracy: 93.8,
        fairnessMetrics: { demographicParity: 0.93, equalizedOdds: 0.9, calibration: 0.94 },
        limitations: [
          'May produce false positives during lifestyle changes',
          'Requires continuous data stream',
        ],
        intendedUse: 'Health anomaly detection and alerting',
        lastUpdated: Date.now() - 45 * 86400000,
      },
    ]),
  ),

  http.get('*/api/xai/bias', () =>
    ok([
      {
        modelId: 'cycle-patterns',
        reportId: 'bias-001',
        ...
        metrics: { demographicParity: 0.95, equalizedOdds: 0.92 },
        recommendations: ['Monitor age-group disparities', 'Add ethnicity-aware calibration'],
      },
      {
        modelId: 'anomaly-detection',
        reportId: 'bias-002',
        generatedAt: Date.now() - 14 * 86400000,
        overallFairness: 0.91,
        metrics: { demographicParity: 0.93, equalizedOdds: 0.9 },
        recommendations: ['Review false positive rates across demographics'],
      },
    ]),
  ),
  ...
  http.post('*/api/privacy/access-request', async () =>
    ok(
      {
        id: `sar-${Date.now()}`,
        type: 'access',
        ...
        estimatedCompletionAt: Date.now() + 30 * 86400000,
      },
      201,
    ),
  ),
  http.post('*/api/privacy/erasure', async () =>
    ok(
      {
        id: `erasure-${Date.now()}`,
        type: 'erasure',
  ...
  http.post('*/api/privacy/portability', async () =>
    ok(
      {
        id: `port-${Date.now()}`,
        type: 'portability',
        status: 'processing',
        requestedAt: Date.now(),
        estimatedCompletionAt: Date.now() + 7 * 86400000,
        format: 'json',
      },
      201,
    ),
