...
const routes: Array<{ method: string; pattern: RegExp; handler: Handler }> = [
  // ── Wallet ──────────────────────────────────────────────────────────────
  {
    ...
    handler: (url) => {
      const address =
        url.searchParams.get('address') || '0x0000000000000000000000000000000000000000';
      return ok({
    ...
    handler: (url) => {
      ...
      if (search) {
        ...
        filtered = filtered.filter(
          (r) => r.label.toLowerCase().includes(q) || r.provider.toLowerCase().includes(q),
        );
      }
      const start = (page - 1) * limit;
      return paginated(filtered.slice(start, start + limit), {
        page,
        limit,
  ...
  },
  {
    ...
    handler: (url) => {
      ...
      const filtered =
        status && status !== 'all' ? grants.filter((g) => g.status === status) : grants;
  ...
  // ── TEE Explorer ──────────────────────────────────────────────────────
  {
    ...
    handler: (url) => {
      ...
      if (view === 'stats') {
        return ok({ totalEnclaves: 5, activeEnclaves: 4, totalAttestations: 1200, totalJobs: 340 });
      }
    ...
    handler: (url) => {
      ...
      return ok({
        ...
        messages: Array.from({ length: 4 }, (_, i) => ({
          ...
          content:
            i % 2 === 0
              ? 'Analyze my recent cycle data'
              : 'Your cycle data shows consistent 28-day patterns with normal BBT shifts.',
          timestamp: Date.now() - (4 - i) * 60000,
          attestation: i % 2 === 1 ? `0x${'b'.repeat(64)}` : undefined,
          model: i % 2 === 1 ? 'Cycle Pattern Analysis' : undefined,
  ...
  },
  {
    ...
    handler: () =>
      ok({
        ...
        role: 'assistant',
        content:
          'Based on your health data processed in the TEE enclave, your cycle patterns appear consistent with healthy variation.',
        timestamp: Date.now(),
  ...
  // ── Vault ───────────────────────────────────────────────────────────────
  {
    ...
    handler: (url) => {
      ...
      return ok({
        ...
        encryptionKey: `0x${'f'.repeat(64)}`,
        accessListCount: 2,
        jurisdictionFlags: ['us-ca', 'eu-gdpr'],
    ...
    handler: () => {
      ...
      return ok(
        categories.map((cat, i) => ({
          ...
          lockStatus: i < 3 ? 'locked' : 'unlocked',
          recordCount: 10 + i * 8,
          storageUsed: (50 + i * 20) * 1024 * 1024,
          lastAccessed: Date.now() - i * 86400000,
          encryptionKey: `0x${'f'.repeat(64)}`,
          accessListCount: i % 3,
          jurisdictionFlags: ['us-ca', 'eu-gdpr'],
          createdAt: Date.now() - (180 + i * 30) * 86400000,
        })),
      );
    },
  },
    ...
    handler: () =>
      ok({
        entries: Array.from({ length: 28 }, (_, i) => ({
          ...
          symptoms: [] as string[],
          fertilityScore: i >= 10 && i <= 16 ? 70 + (i - 10) * 5 : 20,
          notes: '',
        })),
        total: 28,
  ...
  },
  {
    ...
    handler: () =>
      ok({
        symptoms: Array.from({ length: 5 }, (_, i) => ({
          id: `sym-${i}`,
          date: Date.now() - i * 86400000,
      ...
      ok([
        {
          ...
          description: 'Standard data access policy',
        ...
        },
        {
          ...
          description: 'Access for approved research',
          scopes: ['anonymized_data'],
          defaultDurationDays: 365,
        },
        {
          id: 'policy-2',
          name: 'Emergency Access',
          description: 'Emergency healthcare data access',
          scopes: ['cycle_data', 'lab_results', 'vitals'],
          defaultDurationDays: 7,
        },
        {
          id: 'policy-3',
          name: 'Specialist Referral',
          description: 'Referral data sharing with specialist',
          scopes: ['lab_results'],
        ...
        },
        {
          id: 'policy-4',
          name: 'Full Access',
          description: 'Comprehensive data access for primary care',
          scopes: ['cycle_data', 'lab_results', 'vitals', 'medications'],
  ...
  },
  {
    ...
    handler: (url) => {
      ...
      return ok({
        id,
        patientAddress: `aeth1${'p'.repeat(38)}`,
        providerAddress: `aeth1${'q'.repeat(38)}`,
        providerName: 'Dr. Provider 0',
        scopes: ['cycle_data'],
        status: 'active',
        grantedAt: Date.now() - 30 * 86400000,
  ...
  },
  {
    ...
    handler: (url) => {
      ...
      return ok({
        id,
        seller: `aeth1${'s'.repeat(38)}`,
        sellerReputation: 92,
        category: 'menstrual_cycles',
        title: 'Anonymized Cycle Dataset',
        description: 'High-quality anonymized cycle tracking data.',
        dataPoints: 1200,
        qualityScore: 94,
      ...
      return ok([
        {
          ...
          dataPointsSynced: 12400,
          batteryLevel: 82,
          firmwareVersion: 'v3.2.1',
          connectedAt: Date.now() - 90 * 86400000,
        },
        {
          id: 'dev-002',
          provider: 'oura',
          ...
          dataPointsSynced: 8700,
          batteryLevel: 65,
          firmwareVersion: 'v2.8.0',
          connectedAt: Date.now() - 60 * 86400000,
        },
        {
          id: 'dev-003',
          provider: 'whoop',
          ...
          dataPointsSynced: 6200,
          batteryLevel: 45,
          firmwareVersion: 'v4.1.0',
          connectedAt: Date.now() - 45 * 86400000,
        },
        {
          id: 'dev-004',
          provider: 'fitbit',
          deviceName: 'Fitbit',
          status: 'disconnected',
          lastSync: Date.now() - 86400000 * 3,
          dataPointsSynced: 3200,
          batteryLevel: 12,
        ...
        },
        {
          ...
          deviceName: 'Garmin',
          status: 'disconnected',
          lastSync: Date.now() - 86400000 * 7,
          dataPointsSynced: 1500,
          batteryLevel: null,
  ...
  },
  {
    ...
    handler: (url) => {
      ...
      return ok({
        id: `dev-${provider}`,
        provider,
        deviceName: `${String(provider).replace('_', ' ')} Device`,
        status: 'connected',
        lastSync: Date.now() - 3600000,
        dataPointsSynced: 5000,
        connectedAt: Date.now() - 60 * 86400000,
      });
    },
  },
      ...
      ok([
        {
          ...
          completedAt: Date.now() - 86400000 * 2 + 300000,
          errors: [],
        },
        {
          id: 'import-002',
          ...
          completedAt: Date.now() - 86400000 * 5 + 600000,
          errors: ['Invalid Observation format', 'Missing required field', 'Duplicate resource'],
        },
        {
          id: 'import-003',
          source: 'FHIR Bundle Upload',
          resourceCount: 10,
          processedCount: 0,
          failedCount: 0,
          status: 'processing',
      ...
      ok([
        {
          ...
          fieldMappings: [{ fhirPath: 'Observation.value', shioraField: 'value' }],
          isDefault: true,
        },
        {
          id: 'map-002',
          ...
          fieldMappings: [{ fhirPath: 'DiagnosticReport.result', shioraField: 'results' }],
          isDefault: true,
        },
        {
          id: 'map-003',
          fhirResourceType: 'MedicationRequest',
          shioraRecordType: 'prescription',
          fieldMappings: [{ fhirPath: 'MedicationRequest.medication', shioraField: 'medication' }],
          isDefault: true,
        },
      ]),
  },
      ...
      ok([
        {
          ...
          destination: 'ipfs',
          lastExportAt: Date.now() - 86400000,
        },
      ]),
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 10 }, (_, i) => ({
          id: `hist-${i}`,
          alertId: `alert-${i % 5}`,
          metric: ['temperature', 'heart_rate', 'spo2', 'cycle_length', 'hrv'][i % 5],
          severity: ['critical', 'warning', 'info'][i % 3],
          timestamp: Date.now() - i * 86400000,
          resolved: i > 5,
          resolvedAt: i > 5 ? Date.now() - (i - 2) * 86400000 : undefined,
        })),
      ),
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 5 }, (_, i) => ({
          ...
          currentValue: [101.2, 105, 93, 38, 18][i],
          threshold: [100.4, 100, 94, 35, 20][i],
    ...
    handler: (url) => {
      ...
      return ok(
        Array.from({ length: 4 }, (_, i) => ({
          ...
          description: [
            'Age is within 25-35 range',
            'Has verified condition',
            'Data quality score above 90',
            'Provider is TEE-verified',
          ][i],
          proof:
            i < 3
              ? {
  ...
  },
  {
    ...
    handler: () =>
      ok({
        ...
        publicInputs: JSON.stringify({ verified: true }),
        verified: false,
        createdAt: Date.now(),
        expiresAt: Date.now() + 90 * 86400000,
        status: 'proving',
  ...
  // ── Governance ──────────────────────────────────────────────────────────
  {
    ...
    handler: (url) => {
      ...
      return ok({
        id,
        proposer: `aeth1${'g'.repeat(38)}`,
        type: 'feature',
        title: 'Add differential privacy to wearable data',
        description:
          'This proposal seeks to integrate differential privacy mechanisms into the wearable data pipeline.',
        forVotes: 52000,
        againstVotes: 14000,
        abstainVotes: 3500,
        quorum: 100000,
    ...
    handler: (url) => {
      ...
      if (view === 'delegations') {
        return ok([
          {
            id: 'del-0001',
            delegator: `aeth1${'d'.repeat(38)}`,
            delegatee: `aeth1${'e'.repeat(38)}`,
            amount: 2000,
      ...
      const proposals = Array.from({ length: 12 }, (_, i) => ({
        ...
        forVotes: 45000 + i * 5000,
        againstVotes: 12000 + i * 2000,
        abstainVotes: 3000 + i * 500,
        quorum: 100000,
        startBlock: 2847000 - i * 1000,
        endBlock: 2857000 - i * 1000,
        status:
          i < 3
            ? 'active'
            : [
    ...
    handler: () =>
      ok(
        Array.from({ length: 5 }, (_, i) => ({
          ...
          amount: 10 + i * 5,
          currency: 'AETHEL',
          earnedAt: Date.now() - i * 86400000 * 7,
          claimedAt: i < 3 ? Date.now() - i * 86400000 * 5 : undefined,
          epoch: 240 + i,
          txHash: i < 3 ? `0x${'c'.repeat(64)}` : undefined,
        })),
      ),
  },
  {
    ...
    handler: (url) => {
      ...
      return ok(
        Array.from({ length: 3 }, (_, i) => ({
          id: `stake-${String(i).padStart(4, '0')}`,
          staker: `aeth1${'s'.repeat(38)}`,
          amount: 1000 + i * 500,
          stakedAt: Date.now() - (90 + i * 30) * 86400000,
          unlockAt: i === 2 ? Date.now() + 14 * 86400000 : undefined,
          status: i < 2 ? 'staked' : 'unstaking',
          rewardsEarned: 50 + i * 25,
          rewardsClaimed: 30 + i * 15,
          votingPower: 1000 + i * 500,
          txHash: `0x${'b'.repeat(64)}`,
        })),
      );
    },
  ...
  // ── Community ───────────────────────────────────────────────────────────
  {
    ...
    handler: (url) => {
      ...
      return ok({
        ...
        isJoined: true,
        requiresZKP: true,
        zkpClaimType: 'condition_present',
        icon: 'Heart',
        color: '#a78bfa',
      });
    },
    ...
    handler: (url) => {
      ...
      return ok(
        CATEGORIES.map((cat, i) => ({
          ...
          category: cat,
          description:
            CIRCLE_DESCRIPTIONS[cat] || `Support and discussion for ${cat.replace(/_/g, ' ')}`,
          memberCount: 500 + i * 150,
          postCount: 200 + i * 80,
          createdAt: Date.now() - 365 * 86400000,
          isJoined: i < 5,
          requiresZKP: i < 4,
    ...
    handler: (url) => {
      ...
      return ok(
        Array.from({ length: 8 }, (_, i) => ({
          ...
          reactions: [
            { emoji: 'heart', count: 5 + i },
            { emoji: 'support', count: 3 + i },
          ],
          replyCount: i * 2,
          zkpVerified: i % 2 === 0,
    ...
    handler: () =>
      ok(
        Array.from({ length: 10 }, (_, i) => ({
          ...
          amount: [5, 2, 1, 10, 25, 5, 50, 20, 5, 2][i],
          currency: 'AETHEL',
      ...
      if (include === 'streaks') {
        return ok([
          ...
          },
          {
            ...
            currentStreak: 3,
            longestStreak: 6,
            lastActionAt: Date.now() - 86400000 * 7,
            nextMilestone: 4,
            multiplier: 1.0,
          },
        ]);
      ...
      return ok(
        Array.from({ length: 4 }, (_, i) => ({
          ...
          title: [
            ...
            'Menstrual Cycle Variability and Stress',
            'Wearable-Derived Fertility Prediction',
          ...
          participantCount: 50 + i * 30,
          targetParticipants: 200,
          dataTypesRequired: ['lab_result', 'vitals'],
          compensationShio: 50 + i * 25,
          irbApprovalId: `IRB-2024-${1000 + i}`,
          startDate: Date.now() - (180 - i * 30) * 86400000,
          endDate: Date.now() + (180 + i * 30) * 86400000,
          zkpRequired: i < 2,
        })),
      );
    },
  ...
  // ── Research POST ──────────────────────────────────────────────────────
  {
    ...
    handler: () =>
      ok({
        ...
        participantCount: 51,
        targetParticipants: 200,
        dataTypesRequired: ['lab_result'],
        compensationShio: 50,
        irbApprovalId: 'IRB-TEST',
        startDate: Date.now(),
    ...
    handler: () =>
      ok(
        Array.from({ length: 3 }, (_, i) => ({
          ...
          scenario: ['Medication Change', 'Diet Adjustment', 'Exercise Plan'][i],
  ...
  },
  {
    ...
    handler: () =>
      ok({
        ...
        description: 'User-initiated simulation.',
        status: 'running',
        startedAt: Date.now(),
        attestation: `0x${'a'.repeat(64)}`,
      }),
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 5 }, (_, i) => ({
          id: `param-${i}`,
          name: ['Weight', 'Height', 'BMI', 'Heart Rate', 'Sleep Hours'][i],
          category: ['physical', 'physical', 'physical', 'vitals', 'lifestyle'][i],
          currentValue: [70, 170, 24.2, 72, 7.5][i],
          unit: ['kg', 'cm', 'kg/m²', 'bpm', 'hours'][i],
          lastUpdated: Date.now() - i * 86400000,
        })),
      ),
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 4 }, (_, i) => ({
          id: `pred-${i}`,
          biomarker: ['HbA1c', 'Cholesterol', 'TSH', 'Vitamin D'][i],
          currentValue: [5.4, 190, 2.1, 35][i],
          predictedValue: [5.3, 185, 2.0, 38][i],
          confidence: 92 - i * 2,
          timeframedays: 90,
          unit: ['%', 'mg/dL', 'mIU/L', 'ng/mL'][i],
        })),
      ),
  },
  {
  ...
  // ── Clinical Decision Support ─────────────────────────────────────────
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 3 }, (_, i) => ({
          id: `pathway-${i}`,
          name: ['Diabetes Management', 'Hypertension Protocol', 'Asthma Care'][i],
          status: 'active',
          steps: 5 + i,
          completedSteps: 3 + i,
    ...
    handler: () =>
      ok(
        Array.from({ length: 2 }, (_, i) => ({
          id: `interaction-${i}`,
          drugA: ['Metformin', 'Lisinopril'][i],
          drugB: ['Insulin', 'Amlodipine'][i],
          severity: ['moderate', 'mild'][i],
          description: `Drug interaction ${i}`,
          recommendation: `Monitor ${i}`,
        })),
      ),
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 3 }, (_, i) => ({
          id: `diff-${i}`,
          condition: ['Type 2 Diabetes', 'Hypothyroidism', 'Iron Deficiency'][i],
          probability: 0.85 - i * 0.15,
          supportingEvidence: ['Elevated HbA1c', 'Elevated TSH', 'Low ferritin'][i],
        })),
      ),
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 5 }, (_, i) => ({
          ...
          timestamp: Date.now() - i * 86400000,
          actor: `Dr. Provider ${i}`,
          details: `Clinical audit entry ${i}`,
        })),
      ),
  },
  {
    ...
    handler: (url) => {
      ...
      return ok({
        ...
        activeAlerts: 2,
        resolvedAlerts: 8,
        interactionsChecked: 15,
        differentialsGenerated: 12,
  ...
  // ── Compliance ────────────────────────────────────────────────────────
  {
    ...
    handler: (url) => {
      ...
      const items = Array.from({ length: Math.min(limit, 10) }, (_, i) => ({
        ...
        timestamp: Date.now() - i * 86400000,
        details: `Compliance audit entry ${i}`,
      }));
  ...
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 3 }, (_, i) => ({
          id: `report-${i}`,
          title: ['HIPAA Annual Report', 'GDPR Assessment', 'SOC2 Audit'][i],
          framework: ['hipaa', 'gdpr', 'soc2'][i],
          status: ['completed', 'completed', 'in_progress'][i],
          generatedAt: Date.now() - i * 30 * 86400000,
          score: 95 - i * 3,
        })),
      ),
  },
    ...
    handler: () =>
      ok(
        Array.from({ length: 6 }, (_, i) => ({
          ...
          status: i < 4 ? 'passed' : i < 5 ? 'failed' : 'not_applicable',
          lastChecked: Date.now() - i * 86400000,
          framework: 'hipaa',
        })),
      ),
  },
  {
    ...
    handler: (url) => {
      ...
      if (view === 'frameworks') {
        return ok(
          Array.from({ length: 3 }, (_, i) => ({
            id: ['hipaa', 'gdpr', 'soc2'][i],
            name: ['HIPAA', 'GDPR', 'SOC 2'][i],
            complianceScore: 95 - i * 3,
            totalControls: 20 + i * 5,
            passingControls: 18 + i * 3,
            lastAssessment: Date.now() - i * 30 * 86400000,
          })),
        );
      }
      if (view === 'violations') {
        return ok(
          Array.from({ length: 2 }, (_, i) => ({
            id: `violation-${i}`,
            framework: ['hipaa', 'gdpr'][i],
            severity: ['high', 'medium'][i],
            title: `Violation ${i}`,
            description: `Policy violation ${i}`,
            status: ['open', 'remediated'][i],
  ...
  // ── Emergency ─────────────────────────────────────────────────────────
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 4 }, (_, i) => ({
          id: `member-${i}`,
          name: ['Dr. Sarah Chen', 'Dr. James Liu', 'Nurse Kim Park', 'Dr. Emily Davis'][i],
  ...
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 3 }, (_, i) => ({
          id: `protocol-${i}`,
          name: ['Cardiac Emergency', 'Allergic Reaction', 'Stroke Protocol'][i],
          severity: ['critical', 'high', 'critical'][i],
          steps: 5 + i,
  ...
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 2 }, (_, i) => ({
          id: `handoff-${i}`,
          fromProvider: `Dr. Provider ${i}`,
          toProvider: `Dr. Provider ${i + 1}`,
          patientSummary: `Patient handoff summary ${i}`,
          status: ['completed', 'in_progress'][i],
  ...
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 3 }, (_, i) => ({
          id: `greport-${i}`,
          category: ['pharmacogenomics', 'risk_assessment', 'biomarkers'][i],
    ...
    handler: (url) => {
      ...
      if (view === 'pharmacogenomics') {
        return ok(
          Array.from({ length: 4 }, (_, i) => ({
            id: `pgx-${i}`,
            gene: ['CYP2D6', 'CYP2C19', 'CYP3A4', 'SLCO1B1'][i],
            phenotype: [
              'Normal Metabolizer',
              'Rapid Metabolizer',
              'Poor Metabolizer',
      ...
      if (view === 'risk-scores') {
        return ok(
          Array.from({ length: 3 }, (_, i) => ({
            id: `prs-${i}`,
            condition: ['Type 2 Diabetes', 'Coronary Artery Disease', 'Breast Cancer'][i],
            score: [0.35, 0.22, 0.12][i],
            percentile: [72, 55, 30][i],
      ...
      return ok({
        totalVariants: 2500,
        clinicallyRelevant: 42,
        pharmacogenomicGenes: 4,
        riskScoresCalculated: 3,
    ...
    handler: () =>
      ok(
        Array.from({ length: 4 }, (_, i) => ({
          id: `mpc-session-${i}`,
          name: `MPC Session ${i}`,
          protocol: ['SPDZ', 'ABY3', 'SPDZ', 'ABY3'][i],
          status: ['completed', 'running', 'completed', 'pending'][i],
          participants: 3 + i,
          createdAt: Date.now() - i * 7 * 86400000,
        })),
      ),
  },
  {
  ...
  },
  {
    ...
    handler: () =>
      ok(
        Array.from({ length: 3 }, (_, i) => ({
          id: `mpc-result-${i}`,
          sessionId: `mpc-session-${i}`,
          accuracy: 0.92 + i * 0.02,
          completedAt: Date.now() - i * 7 * 86400000,
    ...
    handler: () =>
      ok(
        Array.from({ length: 3 }, (_, i) => ({
          id: `mpc-dataset-${i}`,
          name: `Dataset ${i}`,
          recordCount: 1000 + i * 500,
          features: 10 + i * 3,
          encrypted: true,
          createdAt: Date.now() - i * 30 * 86400000,
        })),
      ),
  },
    ...
    handler: (url) => {
      if (url.searchParams.get('type') === 'fertility') {
        return ok(
          Array.from({ length: 4 }, (_, i) => ({
            id: `fertility-${i}`,
            name: ['LH Surge', 'Cervical Mucus', 'BBT Shift', 'Progesterone'][i],
            value: [25, 3, 0.5, 12][i],
            unit: ['mIU/mL', 'score', '°F', 'ng/mL'][i],
            date: Date.now() - i * 86400000,
            status: ['detected', 'fertile', 'confirmed', 'normal'][i],
          })),
        );
      }
      ...
      return ok({
        compartments: 8,
        totalRecords: 342,
        storageUsed: 1.8 * 1024 * 1024 * 1024,
        privacyScore: {
          overall: 92,
          encryptionScore: 98,
    ...
    handler: () =>
      ok(
        Array.from({ length: 5 }, (_, i) => ({
          ...
          overallScore: 95 - i * 5,
          reviewCount: 20 + i * 5,
          totalAccesses: 100 + i * 30,
          onTimeRevocations: 98 - i,
          dataBreaches: 0,
          averageAccessDuration: 30 + i * 10,
          registeredAt: Date.now() - (365 + i * 90) * 86400000,
          lastActivityAt: Date.now() - i * 86400000 * 3,
        })),
      ),
  },
      ...
      return ok([
        {
          ...
          confidence: 96.2,
          explanation: 'The workload estimates normal ovulation timing.',
        },
        {
          inferenceId: 'inf-002',
      ...
      ok([
        {
          ...
          name: 'Cycle Pattern Analysis',
          version: 'v2.1',
          description: 'Estimates menstrual cycle timing.',
        ...
        },
        {
          ...
          version: 'v1.4',
          description: 'Detects unusual health patterns.',
  ...
  },
  {
    ...
    handler: () =>
      ok([
        { modelId: 'cycle-patterns', reportId: 'bias-001', overallFairness: 0.94 },
        { modelId: 'anomaly-detection', reportId: 'bias-002', overallFairness: 0.91 },
      ]),
  },
  ...
  // ── Wallet Session ──────────────────────────────────────────────────────
  {
    ...
    pattern: /\/api\/wallet\/connect$/,
    handler: () =>
      ok({ address: '0x00000000000000000000000000000000000a1b2c', authenticated: true }),
  },
  {
    method: 'DELETE',
    pattern: /\/api\/wallet\/connect$/,
  ...
  },
  {
    ...
    pattern: /\/api\/ipfs\/upload/,
    handler: (url) => {
      if (url.searchParams.get('metrics')) {
        return ok({
  ...
  },
  {
    ...
    handler: () =>
      ok({
        ...
        size: 1024 * 50,
        metadata: { encrypted: true, encryption: 'AES-256-GCM' },
      }),
  },
...
global.fetch = jest.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  return createMockResponse(
    {
      success: false,
      error: { code: 'NOT_FOUND', message: `No mock handler for ${method} ${url.pathname}` },
    },
    404,
  );
}) as jest.Mock;
