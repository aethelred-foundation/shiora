...
const DEFAULT_CARE_TEAM = [
  ...
  },
  {
    ...
    accessLevel: 'partial' as const,
    lastInteraction: Date.now() - 259200000,
    isActive: true,
  },
  {
    id: 'member-3',
    ...
    lastInteraction: Date.now() - 345600000,
    isActive: false,
  },
];
...
const DEFAULT_PROTOCOLS = [
  {
    ...
    steps: [{ order: 1, instruction: 'Call 911' }],
    autoNotifyTeam: true,
  ...
  },
  {
    ...
    steps: [{ order: 1, instruction: 'Administer epinephrine' }],
    autoNotifyTeam: true,
  ...
  },
  {
    ...
    category: 'neuro',
    severity: 'critical' as const,
    steps: [{ order: 1, instruction: 'FAST assessment' }],
    autoNotifyTeam: true,
    teeVerifiedDoses: true,
    attestation: '0xjkl',
...
const DEFAULT_TRIAGE_HISTORY = [
  {
    id: 'triage-0',
    symptoms: ['chest_pain', 'shortness_of_breath', 'dizziness', 'nausea'],
    vitalSigns: { heartRate: 102 },
    esiLevel: 2 as const,
    disposition: 'emergency_room' as const,
    reasoning: 'Urgent',
    confidence: 95,
    attestation: '0x111',
    assessedAt: Date.now() - 7 * 86400000,
    modelId: 'model-v1',
  },
  {
    id: 'triage-1',
    symptoms: ['headache'],
    vitalSigns: { heartRate: 78 },
    esiLevel: 4 as const,
    disposition: 'home_care' as const,
    reasoning: 'Non-urgent',
    confidence: 78,
    attestation: '0x222',
    assessedAt: Date.now() - 14 * 86400000,
    modelId: 'model-v1',
  },
  {
    id: 'triage-2',
    ...
    confidence: 85,
    attestation: '0x333',
    assessedAt: Date.now() - 21 * 86400000,
    modelId: 'model-v1',
  },
];
...
const DEFAULT_HANDOFFS = [
  {
    id: 'handoff-0',
    fromProvider: 'Dr. Provider 0',
    toProvider: 'Dr. Provider 1',
    patientSummary: 'Patient handoff summary 0',
    outstandingIssues: ['Issue 1'],
    medications: ['Med A'],
    qualityScore: 92,
    completenessScore: 88,
    handoffAt: Date.now() - 86400000,
    acknowledgedAt: Date.now() - 43200000,
  },
  {
    id: 'handoff-1',
    fromProvider: 'Dr. Provider 1',
    toProvider: 'Dr. Provider 2',
    patientSummary: 'Patient handoff summary 1',
    outstandingIssues: [],
    medications: ['Med B'],
    qualityScore: 85,
    completenessScore: 90,
    handoffAt: Date.now() - 259200000,
  },
];
...
describe('EmergencyPage', () => {
  ...
  it('renders the page description', () => {
    ...
    expect(
      screen.getByText(
        /Emergency medical information, care team management, and structured triage/,
      ),
    ).toBeInTheDocument();
  });
  ...
  it('shows empty state when no emergency card is configured', () => {
    ...
    expect(
      screen.getByText('No emergency card configured. Connect your wallet to set up.'),
    ).toBeInTheDocument();
  });
  ...
  it('switches to Protocols tab and shows protocols', () => {
    ...
    expect(
      screen.getByText(/3 protocols with step-by-step emergency response instructions/),
    ).toBeInTheDocument();
  });
  ...
  it('switches to Triage tab and shows Triage Assessment', () => {
    render(
      <TestWrapper>
        <EmergencyPage />
      </TestWrapper>,
    ...
    expect(screen.getAllByText('Triage Assessment').length).toBeGreaterThanOrEqual(1);
