...
testnet-only until it does, and anchoring is always optional and fail-soft.
...
- `SHIORA_L1_RPC_URL`, `SHIORA_L1_ANCHOR_FROM`, `SHIORA_L1_ANCHOR_TO` — audit
  anchoring broadcast (testnet only for now; local WORM series when unset).
...
| `POST /api/anchors` | Anchor the audit head (admin; scheduler-driven) |
...
- `POST /api/anchors` — periodic audit-head anchoring (optional; fail-soft).
...
- Crypto-shredding interacts with backups: a restored backup may resurrect a
  *sealed* envelope, but shredded DEKs are unrecoverable by design — restoring
