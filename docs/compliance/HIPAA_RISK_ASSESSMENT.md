...
|---|---|---|---|---|---|---|
| R-1 | KEK compromise via env-backed key (no KMS/HSM in default config) | Moderate | High | **High** | Versioned `KeyProvider` seam, rotation; **Vault Transit DEK wrapping adopted through the whole PHI envelope path** (`DekWrapper` seam — configuring Transit moves the master key out of application memory entirely, docs/KEY_MANAGEMENT.md) | **Gap:** production Vault Transit rollout and removing the env-key path in production. Until Transit is deployed, treat the env KEK as the top residual risk. |
