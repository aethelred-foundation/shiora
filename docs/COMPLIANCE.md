...
| **C-ENC-2** Key custody | Keys held in a KMS/HSM, rotated | 🟡 Partial | KEK is env-sourced with a versioned wrap format ready for rotation. **Gap:** delegate `wrapDek`/`unwrapDek` to AWS KMS / GCP KMS / Vault Transit; remove env-key path in production. |
...
| **C-AUTH-2** MFA | Best practice / HITRUST | ⛔ Not started | Wallet signature is single-factor. **Gap:** add WebAuthn/passkey or TOTP second factor for sensitive actions. |
