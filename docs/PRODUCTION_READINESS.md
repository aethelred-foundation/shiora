...
| 🗺️     | **Roadmap integration** — real external system (TEE / ZK / MPC / chain / IPFS / managed inference) to replace a simulated subsystem. |
...
non-durable store. The test suite holds a **100% coverage gate** (303 suites / 4,796 tests).
Production dependency audits for the application and contracts report **zero vulnerabilities**;
the isolated contract-development toolchain retains upstream-only advisories that do not ship in
either runtime.
...
3. **Roadmap integrations (🗺️)** — real TEE, ZK prover, MPC engine, L1 chain client, IPFS, and
   managed inference gateway. Until each is wired, its surface is disabled or explicitly labelled.
...
| Dependency scanning; criticals fixed or risk-accepted | ✅     | Sec         | Production audits: **0 vulnerabilities**; contract dev-tooling advisories are isolated and tracked. |
...
| Managed inference (SANA)  | 🗺️     | Eng   | Configuration-gated and fail-closed; on enablement: no autonomous diagnosis, request/response governance, de-identified inputs, safety filters. |
